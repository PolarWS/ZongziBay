...
const detailItem = ref<API.TMDBTV | null>(null)

// 打开详情模态框（阻止冒泡，避免触发卡片跳转）
const openDetail = (it: API.TMDBTV) => {
  detailItem.value = it
...
// TMDB 名称辅助：原始名（用于搜索）与中文名（用于展示/传递）
const getOriginalKeyword = (it: API.TMDBTV) =>
  (it.original_name && it.original_name.trim()) || (it.name && it.name.trim()) || ''
const getChineseName = (it: API.TMDBTV) =>
  (it.name && it.name.trim()) || (it.original_name && it.original_name.trim()) || ''

// 点击卡片：按当前来源跳转海盗湾 / 动漫花园 / ASSRT字幕站，海盗湾优先用英文标题
const goSearch = async (it: API.TMDBTV) => {
...
<template>
  <div class="px-2 md:px-0">
    <div class="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-lg font-semibold truncate min-w-0">剧集搜索：{{ q || '—' }}</h1>
    ...
    />
    <MediaDetailDialog v-model:open="detailOpen" :media="detailItem" type="tv" />
  </div>
</template>
